...
{
#ifdef CONFIG_MACH_ACER_A4
	if( type == EV_KEY ) {  // direct key input event
		...
		if( handle &&
		    handle->dev &&
		    handle->dev->name &&
		    !strcmp(handle->dev->name, CYPRESS_TS_DRIVER_NAME) )  // touch input event
			schedule_work(&dbs_refresh_work);
#else
		schedule_work(&dbs_refresh_work);
#endif  // CONFIG_TOUCHSCREEN_CYPRESS
	}
#else  // CONFIG_MACH_ACER_A4
	schedule_work(&dbs_refresh_work);
