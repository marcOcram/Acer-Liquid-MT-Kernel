...
{
	wake_lock_timeout(&delay_wlock, HZ>>1);  // keep 0.5 seconds
	wake_unlock(lock);
}
#endif
...
static void usb_chg_detect(struct work_struct *w)
{
	...
	spin_unlock_irqrestore(&ui->lock, flags);
#ifdef CONFIG_MACH_ACER_A4
	batt_chg_type_notify_callback((uint8_t)ui->chg_type);
#endif
...
#ifdef CONFIG_MACH_ACER_A4
static DEVICE_ATTR(usb_state, S_IRUSR | S_IROTH, show_usb_state, 0);
static DEVICE_ATTR(usb_speed, S_IRUSR | S_IROTH, show_usb_speed, 0);
static DEVICE_ATTR(chg_type, S_IRUSR | S_IROTH, show_usb_chg_type, 0);
static DEVICE_ATTR(chg_current, S_IWUSR | S_IRUSR | S_IROTH | S_IWOTH,
