...
/* ATHENV +++ */	
	printk(KERN_ERR "%s: %d\n", __func__, irq);
	printk(KERN_ERR "%s: +msmsdcc_check_status\n", __func__);
/* ATHENV --- */
...
#ifdef CONFIG_MACH_ACER_A4
	if ((host->pdev_id == 4) && (host->eject == 1))
		if (host->plat->sd_setup)
			host->plat->sd_setup(1);
#endif
...
#ifdef CONFIG_MACH_ACER_A4
	if ((host->pdev_id == 4) && (host->eject == 0))
		if (host->plat->sd_setup)
			host->plat->sd_setup(1);
#endif
