...
#ifdef CONFIG_MACH_ACER_A4
	tail = (file_length > (DUMP_APANIC_PAGES * ctx->mtd->writesize)) ?
			((file_length / ctx->mtd->writesize) - DUMP_APANIC_PAGES + 1)  : 0;
	page_no = ((file_offset + offset) / ctx->mtd->writesize) + tail;
...
#ifdef CONFIG_MACH_ACER_A4
	if (offset > ((page_no - tail) * ctx->mtd->writesize - file_offset))
		len = ((page_no + 1 - tail) * ctx->mtd->writesize - file_offset) - offset;
	else
		len = count;
	memcpy(buffer, ctx->bounce + page_offset, len);
	*start = (char *) len;
