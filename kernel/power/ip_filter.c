		...
		for (i = 0; i < UDP_HTABLE_SIZE; i++) {
			...
			sk_nulls_for_each(sk, node, &hslot->head) {
				if (sk->sk_family == PF_INET)
					...
					ACER_DBG("UDP Not Match!!! sk->sk_family = 0x%x, PF_INET=0x%x\n", sk->sk_family, PF_INET);
			}
			...
			if (si != NULL) {
				if (count < MAX_ALLOW_TABLE_SIZE) {
					...
					ACER_DBG("UDP Table Full! Now is %d\n", count);
					break;
				}
			}
			si = NULL;
...
int get_tcp_table(packet_item *ptable, int *pcount)
{
	...
	if (!ptable || !pcount) {
		ACER_DBG("NULL tcp_table or count!\n");
		return -1;
	}
...
int send_table(packet_item *ptable, int count)
{
	...
	ACER_DBG("Send Allow Table++\n");
	if (!ptable) {
		ACER_DBG("ptable = NULL!\n");
		return -1;
	}
...
#if SEND_DST_PORT
				ACER_DBG("Send UDP DST_PORT: port_local = %d\n", ptable[i].port_local);
...
#else
				ACER_DBG("Skip UDP DST_PORT: port_local = %d\n", ptable[i].port_local);
#endif
			}
			break;
		case PROT_TCP:
			if (ptable[i].ip_local && ptable[i].ip_foreign) {
				if (ptable[i].ip_local == 0x7F000001) {
					...
					/* Add TCP Port */
					ACER_DBG("Skip TCP PORT (127.0.0.1): port_local = %d\n", ptable[i].port_local);

				} else {
					/* Add TCP Src IP */
#if SEND_SRC_IP
					ACER_DBG("Send TCP SRC IP: ip_foreign = %d.%d.%d.%d\n",
...
#else
					ACER_DBG("Skip TCP SRC IP: ip_foreign = %d.%d.%d.%d\n",
						(unsigned char)(ptable[i].ip_foreign >> 24),
						(unsigned char)(ptable[i].ip_foreign >> 16),
						(unsigned char)(ptable[i].ip_foreign >> 8),
						(unsigned char)(ptable[i].ip_foreign >> 0));
...
#if SEND_DST_PORT
				ACER_DBG("Send TCP PORT (Has IP): port_local = %d\n", ptable[i].port_local);
				...
				req_packet_port.item_count++;
#else
				ACER_DBG("Skip TCP PORT (Has IP): port_local = %d\n", ptable[i].port_local);
#endif
				}
			} else {
				/* Add TCP Port */
#if SEND_DST_PORT
				ACER_DBG("Send TCP PORT LISTENING: port_local = %d\n", ptable[i].port_local);
...
#else
				ACER_DBG("Skip TCP PORT LISTENING: port_local = %d\n", ptable[i].port_local);
#endif
			}
			break;
		}
...
void print_allow_table(void)
{
	...
	ACER_DBG("\nPrint Allow Table\n");
	ACER_DBG("PROT  Local Address  Foreign Address   State\n");
	for (i = 0; i < allow_table_count; i++) {
		...
		case PROT_TCP:
			...
			break;
		default:
			ACER_DBG("0x%x    ", packet_allow_table[i].protocol);
		}
		ACER_DBG("%d.%d.%d.%d:%d    %d.%d.%d.%d:%d    %d\n",
