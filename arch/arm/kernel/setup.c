...
#if defined(CONFIG_BOARD_KRAMLOG_BASE) && defined(CONFIG_BOARD_KRAMLOG_SIZE)
	if (bank->start == (unsigned long) CONFIG_BOARD_KRAMLOG_BASE && bank->size == (unsigned long) CONFIG_BOARD_KRAMLOG_SIZE) {
		kramlog_phys = CONFIG_BOARD_KRAMLOG_BASE;
		kramlog_size = CONFIG_BOARD_KRAMLOG_SIZE;
		return 0;
	}
#endif
