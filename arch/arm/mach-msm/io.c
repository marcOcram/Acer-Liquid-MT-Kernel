...
unsigned int kramlog_phys = CONFIG_BOARD_KRAMLOG_BASE;
unsigned int kramlog_size = 0;
EXPORT_SYMBOL(kramlog_phys);
EXPORT_SYMBOL(kramlog_size);
#endif
...
static void msm_map_io(struct map_desc *io_desc, int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		if (io_desc[i].virtual == (unsigned long) ACER_KRAMLOG_BASE)
			io_desc[i].pfn = __phys_to_pfn(kramlog_phys);
	}
#else
	for (i = 0; i < size; i++)
		if (io_desc[i].virtual == (unsigned long)MSM_SHARED_RAM_BASE)
...
#if defined(CONFIG_BOARD_KRAMLOG_BASE) && defined(CONFIG_BOARD_KRAMLOG_SIZE)
	{
		.virtual =  (unsigned long) ACER_KRAMLOG_BASE,
		.length =   CONFIG_BOARD_KRAMLOG_SIZE,
		.type =     MT_DEVICE,
	},
#endif
};
