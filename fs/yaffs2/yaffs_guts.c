...
#ifdef CONFIG_MACH_ACER_A4
	if (dev == NULL || dev->unlinkedDir == NULL) {
		pr_err("[yaffs_StripDeletedObjects] dev NULL... 111\r\n");
		return ;
	}

	t = &dev->unlinkedDir->variant.directoryVariant.children;
	if (t->next == NULL || t->next->next == NULL) {
		pr_err("[yaffs_StripDeletedObjects] t->next NULL... 111\r\n");
		return ;
	}
#endif
...
#ifdef CONFIG_MACH_ACER_A4
	if (dev == NULL || dev->deletedDir == NULL) {
		pr_err("[yaffs_StripDeletedObjects] dev NULL... 222\r\n");
		return ;
	}

	t = &dev->deletedDir->variant.directoryVariant.children;
	if (t->next == NULL || t->next->next == NULL) {
		pr_err("[yaffs_StripDeletedObjects] t->next NULL... 222\r\n");
		return ;
	}
#endif
